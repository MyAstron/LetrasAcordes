...
class CancionesViewModel(
    ...
    fun generarPdf(canciones: List<Cancion>, withChords: Boolean, includeIndex: Boolean, compactMode: Boolean, uri: Uri) {
        viewModelScope.launch(Dispatchers.IO) {
            pdfGenerator.generateSongbookPdf(
                songs = canciones, 
    ...
    // Exporta comprimido en GZIP (Binario)
    ...
    // Importa detectando si es GZIP o Texto plano (Retrocompatibilidad)
    suspend fun importarCanciones(datosArchivo: ByteArray): Int {
        val textoImportado = try {
            // Intentar descomprimir GZIP
            ByteArrayInputStream(datosArchivo).use { bis ->
                GZIPInputStream(bis).bufferedReader(StandardCharsets.UTF_8).use { it.readText() }
            }
        } catch (e: Exception) {
            // Si falla, asumimos que es un archivo antiguo de texto plano sin comprimir
